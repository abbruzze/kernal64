...
#KERNAL64C128C64OPTS+=" --shell"
KERNAL64C128C64OPTS+=" --ignore-config-file"
KERNAL64C128C64OPTS+=" --go64"
KERNAL64C128C64OPTS+=" --testcart"
KERNAL64C128C64OPTS+=" --headless"
KERNAL64C128C64OPTS+=" --warp"
KERNAL64C128C64OPTS+=" --vic-palette pepto"
...
# $1  option
# $2  test path
function kernal64c128c64_get_options
{
    exitoptions=""
    case "$1" in
        "default")
...
# $1  option
# $2  test path
function kernal64c128c64_get_cmdline_options
{
    exitoptions=""
}

# called once before any tests run
function kernal64c128c64_prepare
{
    true
}
...
function kernal64c128c64_run_screenshot
{
    if [ "$2" == "" ] ; then
...
#    echo exitcode:$exitcode
    ...
    then
        ...
        then
            if [ $exitcode -ne 255 ]
            then
                echo -ne "\nerror: call to $KERNAL64C128C64 failed.\n"
#                exit -1
            fi
        fi
    fi
...
# $4  test full path+name (may be empty)
# $5- extra options for the emulator
function kernal64c128c64_run_exitcode
{
    if [ $verbose == "1" ]; then
