...
; -----------------------------------------------------------------------------
; temporary pointers
val TMP0    = $C1               ; used to return input, often holds end address
val TMP2    = $C3               ; usually holds start address

; -----------------------------------------------------------------------------
; kernal variables
val SATUS   = $90               ; kernal i/o status word
val FNLEN   = $B7               ; length of current filename
val SADD    = $B9               ; current secondary address (official name SA)
val FA      = $BA               ; current device number
val FNADR   = $BB               ; pointer to current filename
val NDX     = $C6               ; number of characters in keyboard buffer
val KEYD    = $0277             ; keyboard buffer
val BKVEC   = $0316             ; BRK instruction vector (official name CBINV)

        *= $0100 virtual "VARIABLES"           ; store variables in tape error buffer

; -----------------------------------------------------------------------------
; variables
ACMD:    .byte 0             ; addressing command
LENGTH:  .byte 0             ; length of operand
MNEMW:   .fill 3             ; 3 letter mnemonic buffer
SAVX:    .byte 0             ; 1 byte temp storage, often to save X register
OPCODE:  .byte 0             ; current opcode for assembler/disassembler
UPFLG:   .byte 0             ; flag: count up (bit 7 clear) or down (bit 7 set)
DIGCNT:  .byte 0             ; digit count
INDIG:   .byte 0             ; numeric value of single digit
NUMBIT:  .byte 0             ; numeric base of input
...
ESTAGE:						 ; end of staging buffer
        ...
        *= $0200 virtual "BUFFER"            ; store more variables in basic line editor buffer
...
PCH:     .byte 0             ; program counter high byte
PCL:     .byte 0             ; program counter low byte
SR:      .byte 0             ; status register
ACC:     .byte 0             ; accumulator
XR:      .byte 0             ; X register
YR:      .byte 0             ; Y register
SP:      .byte 0             ; stack pointer

STORE:   .byte 0,0           ; 2-byte temp storage
CHRPNT:  .byte 0             ; current position in input buffer
SAVY:    .byte 0             ; temp storage, often to save Y register
U9F:     .byte 0             ; index into assembler work buffer

; -----------------------------------------------------------------------------
; kernal entry points
val SETMSG  = $FF90             ; set kernel message control flag
val SECOND  = $FF93             ; set secondary address after LISTEN
val TKSA    = $FF96             ; send secondary address after TALK
val LISTEN  = $FFB1             ; command serial bus device to LISTEN
val TALK    = $FFB4             ; command serial bus device to TALK
val SETLFS  = $FFBA             ; set logical file parameters
val SETNAM  = $FFBD             ; set filename
val ACPTR   = $FFA5             ; input byte from serial bus
val CIOUT   = $FFA8             ; output byte to serial bus
val UNTLK   = $FFAB             ; command serial bus device to UNTALK
val UNLSN   = $FFAE             ; command serial bus device to UNLISTEN
val CHKIN   = $FFC6             ; define input channel
val CLRCHN  = $FFCC             ; restore default devices
val INPUT   = $FFCF             ; input a character (official name CHRIN)
val CHROUT  = $FFD2             ; output a character
val LOAD    = $FFD5             ; load from device
val SAVE    = $FFD8             ; save to device
val STOP    = $FFE1             ; check the STOP key
val GETIN   = $FFE4             ; get a character
...
ASHIFT:  LSR                ;   into the first two bytes of the inst buffer
...
INSTXX:  TAY                 ; stash opcode in accumulator in Y for later
        ...
        ORA #$80
IEVEN:   LSR                ; LSB determines whether to use left/right nybble
