...
object InspectPanel {
  def getInspectDialog(f: JFrame, root: C64Component) = {
...
class InspectPanel(root: C64Component) extends JPanel with Runnable with ChangeListener {
  ...
  private[this] class ComponentNode(val node: C64Component) {
  ...
  private def createTree(node: C64Component): DefaultMutableTreeNode = {
