  ...
  private def executeCommand(cmd:String) {
    ...
    command.charAt(0) match {
      ...
      case 'U' =>
        ...
        if (command.startsWith("U1")) memoryRead(command.substring(2))
        else setStatus(STATUS_SYNTAX_ERROR)
      case 'B' => 
        if (command.startsWith("B-P:")) bufferPointer(command.substring(4))
        else
        if (command.startsWith("B-P")) bufferPointer(command.substring(3))
        else setStatus(STATUS_SYNTAX_ERROR)
      case 'M' =>
        if (command.startsWith("M-R:")) m_r(command.substring(4))
        else
        if (command.startsWith("M-R")) m_r(command.substring(3))
        else
        if (command.startsWith("M-W:")) m_w(command.substring(4))
        else
        if (command.startsWith("M-W")) m_w(command.substring(3))
        else
        if (command.startsWith("M-E:")) m_e(command.substring(4))
        else
        if (command.startsWith("M-E")) m_e(command.substring(3))
      case _ =>
  ...
  private def memoryRead(cmd:String) {
    ...
    if (st.countTokens == 4) {
      try {
        ...
        if (drive != 0) setStatus(STATUS_DRIVE_NOT_READY)
        else {
          // read block
